...
.
    ...
    with open(html_file) as fp:
        ...
        listing_title = []
        listing_names = soup.find_all('div', class_ = "t1jojoys dir dir-ltr")
        for value in listing_names:
            listing_title.append(value.contents[0])
        # print(listing_title)
        

        money_list = []
        money = soup.find_all('span', class_ = "_tyxjp1")
        for value in money:
            appending = int((value.contents[0][1:]))
            money_list.append(appending)
        # print(money_list)
        
        listing_id_list = []
        list_ids = soup.find_all('div', class_="t1jojoys dir dir-ltr")
        for id in list_ids:
            listing_id = id.get("id")
            listing_id_list.append(listing_id[6:])
        # print(listing_id_list)

        final_tuple = list(zip(listing_title,money_list,listing_id_list))
        return final_tuple
...
.
    ...
    with open(file) as fp:
        ...
        policy_number = policy.find("span", class_= "ll4r2nl dir dir-ltr").contents[0]
        # print(policy_number)


        place_type = []
        place = soup.find('h2', class_ = "_14i3z6h").contents[0]
        if "private" in place.lower():
            place_type.append("Private Room")
        elif "shared" in place.lower():
            place_type.append("Shared Room")
        else:
        ...
        final_bed = []
        values = soup.find("ol", class_ = "lgx66tx dir dir-ltr")
        num_of_bedrooms = (values.find_all('li')[1].find_all('span')[2].text)
        if num_of_bedrooms.lower() == "studio":
            final_bed.append(1)
        else:
            final_bed.append(int(num_of_bedrooms[0]))
        # print(final_bed)

        final_tuple = (policy_number, place_type[0], final_bed[0])
        # print(final_tuple)
        return final_tuple
